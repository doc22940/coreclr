...
class StringCompare6
{
    ...
    // new update: add method to generate testId automatically (8-3-2006 by v-yaduoj)
    {
        string temp, testId;
    ...
    {
        StringCompare6 sc = new StringCompare6();

        TestLibrary.TestFramework.BeginTestCase("for method: String.Compare Method (String, String, Boolean)");
        if (sc.RunTests())
        {
    ...
    public bool PosTest1()
    {
        ...
        paras.strA = null;
        paras.strB = null;
        paras.ignoreCase = this.GetBoolean();
    ...
    private bool ExeNegTest_ANE(Parameters paras, string testDesc, string errorDesc)
    {
        bool retVal = true;
        UpdateCounts(TestType.NegativeTest);
        string testId = GenerateTestId(TestType.NegativeTest);// new update
        string testInfo = c_NEG_TEST_PREFIX + this.negTestCount.ToString() + ": " + testDesc;
        TestResult testResult = TestResult.NotRun;
    ...
    {
        return string.Compare(paras.strA, paras.strB, GetComparison(paras.ignoreCase));
    }
    private StringComparison GetComparison(bool ignoreCase)
    {
        StringComparison ret = StringComparison.CurrentCulture;
        if (ignoreCase) ret = StringComparison.CurrentCultureIgnoreCase;
        return ret;
    }
